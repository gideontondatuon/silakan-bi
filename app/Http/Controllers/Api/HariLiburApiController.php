...
{
    /**
     * List holidays with filters and available years.
     */
    public function index(Request $request): JsonResponse
    {
        ...
        $hariLibur = $query->orderBy('tanggal', 'asc')->paginate($request->get('per_page', 15));
    ...
    /**
     * Sync national holidays via API or fallback.
     */
    public function syncApi(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                $count = $this->seedDefaultHolidays((int) $year);
            }
        } catch (\Exception $e) {
            $count = $this->seedDefaultHolidays((int) $year);
        }
        ...
        return response()->json([
            'status' => 'success',
            'message' => "Berhasil sinkronisasi {$count} hari libur nasional & cuti bersama untuk tahun {$year}.",
            'synced_count' => $count,
        ]);
    }
