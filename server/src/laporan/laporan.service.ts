...
export class LaporanService {
  ...
  private buildWhere(query: any) {
    ...
    if (query.jenis_pic) where.jenis_pic = query.jenis_pic;

    // Support both tanggal_mulai/tanggal_selesai (frontend) and from/to (legacy)
    const dateFrom = query.tanggal_mulai || query.from;
    const dateTo = query.tanggal_selesai || query.to;
    if (dateFrom || dateTo) {
      where.tanggal_kegiatan = {};
      if (dateFrom) where.tanggal_kegiatan.gte = new Date(dateFrom);
      if (dateTo) {
